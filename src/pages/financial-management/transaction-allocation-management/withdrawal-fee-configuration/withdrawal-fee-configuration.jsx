...
import { withdrawConfigDetail, withdrawConfigUpdate, youfuWithdrawConfigDetail, youfuWithdrawConfigUpdate } from '@/services/financial-management/withdrawal-fee-configuration'
import { amountTransform } from '@/utils/utils'
import './styles.less'
import styles from './styles.less'

const WithdrawalFeeConfiguration = ({type}) => {
  ...
  useEffect(()=> {
    ...
    if(type === 'xinbao') {
      ...
      youfuWithdrawConfigDetail({}).then(res=> {
        form.setFieldsValue({
          ...res?.data,
          withdrawBusinessFaxScale: amountTransform(res?.data.withdrawBusinessFaxScale, '*'),
          withdrawBusinessFeeFixed: amountTransform(res?.data.withdrawBusinessFeeFixed, '/'),
  ...
  const commit = (val) => {
    ...
    if(flag) {
      ...
      return new Promise((resolve, reject) => {
        if(type === 'xinbao') {
          withdrawConfigUpdate(val, {showSuccess: true}).then(res => {
            if(res.code === 0) {
              resolve()
            } else {
              reject()
          ...
          youfuWithdrawConfigUpdate(val, {showSuccess: true}).then(res => {
            if(res.code === 0) {
              resolve()
            } else {
              reject()
