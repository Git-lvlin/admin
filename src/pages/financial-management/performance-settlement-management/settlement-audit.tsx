        ...
        tableRender={(_, dom) => {
          return <>
            ...
            <div className={styles.summary}>
              <div>
                <p>{type=='1'?'合作公司ID':'账号ID'}：{msgDetail?.applyId}，业绩总分账金额 ：<span style={{ color:'red' }}>{amountTransform(selectedRows.length?selectedRows.reduce((sum, item) => sum + item?.amount, 0):pendingAmount,'/').toFixed(2)}</span>元，扣除通道费：<span style={{ color:'red' }}>{amountTransform(selectedRows.length?selectedRows.reduce((sum, item) => sum + item?.fee, 0):pendingFee,'/').toFixed(2)}</span>元，提成金额 ：<span style={{ color:'red' }}>{amountTransform(selectedRows.length?selectedRows.reduce((sum, item) => sum + item?.unfreezeAmount, 0):pendingUnfreezeAmount,'/').toFixed(2)}</span> 元（ {selectedRows.length?selectedRows.length:totalSum} 单）</p>
              </div>
            </div>
          </>
        }}
